...
class atomic_server(slip.dbus.service.Object):
    ...
    #
    # The version method takes in an image name and returns its version information
    #
    ...
    #
    # The verify method takes in an image name and returns whether or not the image should be updated
    #
    @slip.dbus.polkit.require_auth("org.atomic.read")
    ...
    def verify(self, images):
        ...
        for image in images:
            args=self.Args(str(image))
            self.atomic.set_args(args)
            verifications.append({ "Image" : image, "Verification": self.atomic.verify()})
